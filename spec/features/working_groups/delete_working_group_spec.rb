...
describe "Delete a working group", js: true do
  ...
  let(:working_group) { create(:working_group, :with_admin, :public, circle: circle) }

  let(:circle_wg_page) { PageObject::Circle::WorkingGroups.new }
  let(:edit_wg_form)   { PageObject::WorkingGroup::Form.new }

  let(:inputs) { attributes_for(:working_group, :private, organizer_name: admin.name) }
  ...
  context "deactivated working group" do
    ...
    before { circle_wg_page.deactivate_button.click }
